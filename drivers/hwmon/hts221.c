...
Revision 0.0.4 08/10/2015
 BDU definition update and activation;
 updates on get resolution, oneshot, heater management
...
static int hts221_device_power_off(struct hts221_status *stat)
{
        ...
        if (err < 0)
                dev_err(&stat->client->dev, "humidity soft power off "
                                                        "failed: %d\n", err);
        buf[0] = REG_H_OUT_L;
        err = hts221_i2c_read(stat, buf, 4);
        if (err < 0)
                dev_err(&stat->client->dev, "humidity status reset 1 "
                                                        "failed: %d\n", err);
        buf[0] = status_registers.cntrl2.address;
        buf[1] = 0x00;
        err = hts221_i2c_write(stat, buf, 1);
        if (err < 0)
                dev_err(&stat->client->dev, "humidity status reset 2 "
                                                        "failed: %d\n", err);
...
static int hts221_device_power_on(struct hts221_status *stat)
{
    ...
    #ifdef BDU
       buf[1] = 0x80 | odr_value | REG_DEF_BDU;
    #else
                                                                ...
                                                                int type)
{
        ...
        if(type==0)
                status_registers.h_res.resume_value = buf[1];
        else
                status_registers.t_res.resume_value = buf[1];
...
//       buff[1] = 0x80 | odr_value;
    #ifdef BDU
       buff[1] = ENABLE_SENSOR | odr_value | REG_DEF_BDU;
    #else
                ...
                const char *buf, size_t size)
{
        ...
        if (val!=0) {
          val = 1;
       	  buff[0] = REG_CNTRL1_ADDR;
          buff[1] = DISABLE_SENSOR;//adal:line fixed
          ...
          mutex_lock(&stat->lock);
            ...
            goto unlock_and_exit;
       	  ...
       	  msleep(20);
    #ifdef BDU
          buff[1] = ENABLE_SENSOR | REG_DEF_BDU;
    #else
                ...
                const char *buf, size_t size)
{
    	...
    	if (val == 0) buff[1] = 0x00;
       	...
       	err = hts221_i2c_write(stat, buff, 1);
       	if (!(err<0)) {
       	  atomic_set(&stat->heater, (buff[1]>>1));
       	  pr_info(" Heater Set ");
       	}
       	msleep(20);
...
// Bu default, ODR is set
    ...
    if (odr_value == ODR_ONESH) {
          ...
          do {
          tmp[0] = REG_STATUS;
          err = hts221_i2c_read(stat, tmp, 1);
          if (err < 0)
             return err;
            msleep(45);
          } while(((tmp[0] & 3) != 3) && (3>=cntx++));
          cntx=0;
	}
