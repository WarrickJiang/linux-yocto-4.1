	...
	void (*callback) (void *cookie, struct key_params * params))
{
	...
	memcpy((void *)params.key, (void *)key.data, params.key_len);
...
#if 0 //defined(CONFIG_PM) && defined(WL_CFG80211_P2P_DEV_IF)
	/*
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_JOIN || event == WLC_E_START ||
		...
		if (wl_get_drv_status(cfg, CONNECTED, ndev)) {
			...
			cfg80211_ibss_joined(ndev, (s8 *)&e->addr, notify_channel, GFP_KERNEL);
		}
		else {
			/* New connection */
...
s32 wl_cfg80211_attach(struct net_device *ndev, void *context)
{
	...
	wdev = kzalloc(sizeof(*wdev), GFP_KERNEL);
	if (unlikely(!wdev)) {
		WL_ERR(("wireless device alloc fail, use '__GFP_NOFAIL' to try agian\n"));
		wdev = kzalloc(sizeof(*wdev), GFP_KERNEL | __GFP_NOFAIL);
		if (unlikely(!wdev)) {
			WL_ERR(("Could not allocate wireless device\n"));
			return -ENOMEM;
		}
