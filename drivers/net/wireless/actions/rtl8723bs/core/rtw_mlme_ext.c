...
unsigned int OnAction_back(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	u8 *pframe = precv_frame->u.hdr.rx_data;
	struct sta_priv *pstapriv = &padapter->stapriv;
#ifdef CONFIG_WFD//add by rtk, game miracast for only MI2
	unsigned short peer_ampdu_num = 0 ;
#endif
#ifdef CONFIG_80211N_HT
		...
		switch (action)
		{
			...
			case RTW_WLAN_ACTION_ADDBA_RESP: //ADDBA response
				...
				if ((is_miracast_enabled(padapter->wfd_info.stack_wfd_mode)) && (peer_ampdu_num == 0x8)) {
					...
					ampdu_parm.max_AMPDU_len = 0 ; //0:8k , 1:16k, 2:32k, 3:64k
					report_ampdu_sz_event(padapter, &ampdu_parm) ;
                                }

#endif//end, add by rtk, game miracast for only MI2
