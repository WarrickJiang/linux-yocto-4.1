			...
			if (padapter->registrypriv.wifi_spec) {
				if (process_p2p_cross_connect_ie(padapter, (pframe + WLAN_HDR_A3_LEN), (len - WLAN_HDR_A3_LEN)) == _FALSE) {
					if((padapter->pbuddy_adapter->mlmeextpriv.mlmext_info.state&0x03) == WIFI_FW_AP_STATE) {
						DBG_871X_LEVEL(_drv_notice_, "no issue auth, P2P cross-connect does not permit\n ");
						return _SUCCESS;
					}
				}
			}
...
unsigned int OnDeAuth(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if(check_fwstate(pmlmepriv, WIFI_AP_STATE) == _TRUE)
	{
		...
		DBG_871X_LEVEL(_drv_notice_, FUNC_ADPT_FMT" reason=%u, ta=%pM\n"
...
#endif
	{
		...
		DBG_871X_LEVEL(_drv_notice_, FUNC_ADPT_FMT" reason=%u, ta=%pM, ignore=%d\n"
...
void start_clnt_auth(_adapter* padapter)
{
	...
	DBG_871X_LEVEL(_drv_notice_, "start auth\n");
	issue_auth(padapter, NULL, 0);
	...
	u32 out_num, struct rtw_ieee80211_channel *in, u32 in_num)
{
	...
	for (i=0;i<in_num;i++) {
		...
		{
			if (j >= out_num) {
				DBG_871X_LEVEL(_drv_notice_, FUNC_ADPT_FMT" out_num:%u not enough\n",
...
u8 setkey_hdl(_adapter *padapter, u8 *pbuf)
{
	...
	if (cam_id < 0){
		...
		DBG_871X_LEVEL(_drv_notice_, "set group key camid:%d, addr:"MAC_FMT", kid:%d, type:%s\n"
...
write_to_cam:
	if(pparm->algorithm == _NO_PRIVACY_) {
		while((cam_id = rtw_camid_search(padapter, pparm->addr, -1)) >= 0) {
			DBG_871X_LEVEL(_drv_notice_, "clear key for addr:"MAC_FMT", camid:%d\n", MAC_ARG(pparm->addr), cam_id);
			clear_cam_entry(padapter, cam_id);
			rtw_camid_free(padapter,cam_id);
		}
	} else {
		DBG_871X_LEVEL(_drv_notice_, "set pairwise key camid:%d, addr:"MAC_FMT", kid:%d, type:%s\n",
