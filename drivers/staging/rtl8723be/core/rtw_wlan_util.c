...
int rtw_check_bcn_info(ADAPTER *Adapter, u8 *pframe, u32 packet_len)
{
	...
	{
		DBG_871X_LEVEL(_drv_info_, "%s: start new beacon (seq=%d)\n", __func__, GetSequence(pframe));

		if (pmlmepriv->new_beacon_cnts == 0) {
			DBG_871X_LEVEL(_drv_info_, "%s: cur beacon key\n", __func__);
			DBG_871X_EXP(_drv_err_, rtw_dump_bcn_keys(&pmlmepriv->cur_beacon_keys));
		}

		DBG_871X_LEVEL(_drv_info_, "%s: new beacon key\n", __func__);
	...
	{
		DBG_871X_LEVEL(_drv_info_, "%s: new beacon again (seq=%d)\n", __func__, GetSequence(pframe));
	...
	{
		DBG_871X_LEVEL(_drv_info_, "%s: new beacon occur!!\n", __func__);
