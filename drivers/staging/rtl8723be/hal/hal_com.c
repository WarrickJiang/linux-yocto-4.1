...
*/
	...
	if (adapter->iface_type == IFACE_PORT0) {
		...
		DBG_871X_LEVEL(_drv_notice_, "port switch - port0("ADPT_FMT"), port1("ADPT_FMT")\n",
...
static u8 rtw_hal_check_wow_ctrl(_adapter* adapter, u8 chk_type)
{
	...
	DBG_871X_LEVEL(_drv_notice_, "%s check_type: %d res: %d trycnt: %d\n",
...
static void rtw_hal_force_enable_rxdma(_adapter* adapter)
{
	...
	DBG_871X_LEVEL(_drv_notice_, "%s: Release RXDMA\n", __func__);
...
static u8 rtw_hal_pause_rx_dma(_adapter* adapter)
{
	...
	int res = 0;
	//RX DMA stop
	DBG_871X_LEVEL(_drv_notice_, "Pause DMA\n");
	rtw_write32(adapter, REG_RXPKT_NUM,
			(rtw_read32(adapter,REG_RXPKT_NUM)|RW_RELEASE_EN));
	do{
		if((rtw_read32(adapter, REG_RXPKT_NUM)&RXDMA_IDLE)) {
			DBG_871X_LEVEL(_drv_info_, "RX_DMA_IDLE is true\n");
			ret = _SUCCESS;
			break;
		}
			...
			len = le16_to_cpu(tmp);
			DBG_871X_LEVEL(_drv_info_, "RX len:%d\n", len);

			if (len > 0)
				res = RecvOnePkt(adapter, len);
			else
				DBG_871X_LEVEL(_drv_info_, "read length fail %d\n", len);

			DBG_871X_LEVEL(_drv_info_, "RecvOnePkt Result: %d\n", res);
		}
#endif //CONFIG_SDIO_HCI || CONFIG_GSPI_HCI
	}while(trycnt--);

	if(trycnt ==0) {
		DBG_871X_LEVEL(_drv_info_, "Stop RX DMA failed...... \n");
		ret = _FAIL;
	}
	...
	if(psecuritypriv->binstallKCK_KEK == _TRUE) {
		...
		do{
			...
			if (gtk_keyindex < 4 &&(defualt_cam_id == gtk_keyindex)) {
				...
				DBG_871X_LEVEL(_drv_info_, "GTK (%d) = 0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
						gtk_keyindex,
...
void rtw_hal_set_fw_wow_related_cmd(_adapter* padapter, u8 enable)
{
	...
	DBG_871X_LEVEL(_drv_info_, "+%s()+: enable=%d\n", __func__, enable);
_func_enter_;
...
_func_enter_;
	...
	case HW_VAR_WOWLAN:
	{
		...
		switch (poidparam->subcode) {
			case WOWLAN_ENABLE:
				DBG_871X_LEVEL(_drv_info_, "WOWLAN_ENABLE\n");
				...
				if (res == _FAIL)
					DBG_871X_LEVEL(_drv_info_, "[WARNING] pause RX DMA fail\n");

#if defined(CONFIG_SDIO_HCI) || defined(CONFIG_GSPI_HCI)
				//Enable CPWM2 only.
				res = rtw_hal_enable_cpwm2(adapter);
				if (res == _FAIL)
					DBG_871X_LEVEL(_drv_info_, "[WARNING] enable cpwm2 fail\n");
#endif
#ifdef CONFIG_GPIO_WAKEUP
				...
				DBG_871X_LEVEL(_drv_info_, "Set WOWLan cmd\n");
				...
				break;
			case WOWLAN_DISABLE:
				DBG_871X_LEVEL(_drv_info_, "WOWLAN_DISABLE\n");
				...
				DBG_871X_LEVEL(_drv_info_, "wakeup_reason: 0x%02x\n",
						pwrctl->wowlan_wake_reason);
...
#ifdef CONFIG_GPIO_WAKEUP
				DBG_871X_LEVEL(_drv_info_, "Set Wake GPIO to high for default.\n");
		...
		case HW_VAR_AP_WOWLAN:
		{
			...
			case WOWLAN_AP_ENABLE:
				...
				DBG_871X_LEVEL(_drv_info_, "Re-download WoWlan FW!\n");
#ifdef DBG_CHECK_FW_PS_STATE
				if (rtw_fw_ps_state(adapter) == _FAIL) {
					pdbgpriv->dbg_enwow_dload_fw_fail_cnt++;
					DBG_871X_LEVEL(_drv_info_, "wowlan enable no leave 32k\n");
				}
#endif /*DBG_CHECK_FW_PS_STATE*/
				do {
					if (rtw_read8(adapter, REG_HMETFR) == 0x00) {
						DBG_871X_LEVEL(_drv_info_, "Ready to change FW.\n");
						break;
					}
					rtw_msleep_os(10);
					DBG_871X_LEVEL(_drv_info_, "trycnt: %d\n", (100-trycnt));
				} while (trycnt--);
				...
				/* 2. RX DMA stop*/
				DBG_871X_LEVEL(_drv_info_, "Pause DMA\n");
				trycnt = 100;
				rtw_write32(adapter , REG_RXPKT_NUM ,
					(rtw_read32(adapter , REG_RXPKT_NUM)|RW_RELEASE_EN));
				do {
					...
					/* If RX_DMA is not idle, receive one pkt from DMA*/
					DBG_871X_LEVEL(_drv_info_ , "RX_DMA_IDLE is not true\n");
				} while (trycnt--);
				...
				DBG_871X_LEVEL(_drv_info_, "Set Enable AP WOWLan cmd\n");
			...
			case WOWLAN_AP_DISABLE:
				...
				DBG_871X_LEVEL(_drv_info_, "Set Disable WOWLan cmd\n");
				if (pHalFunc->hal_set_ap_wowlan_cmd != NULL)
				...
				DBG_871X_LEVEL(_drv_info_, "Release RXDMA\n");
		...
		default:
			if (0)
				DBG_871X_LEVEL(_drv_info_, FUNC_ADPT_FMT" variable(%d) not defined!\n",
...
SetHalDefVar(_adapter *adapter, HAL_DEF_VARIABLE variable, void *value)
{
	...
	case HAL_DEF_DBG_DIS_PWT:
		...
		break;
	default:
		DBG_871X_LEVEL(_drv_info_, "%s: [WARNING] HAL_DEF_VARIABLE(%d) not defined!\n", __FUNCTION__, variable);
		bResult = _FAIL;
		break;
	}
...
int rtw_hal_register_gpio_interrupt(_adapter* adapter, int gpio_num, void(*callback)(u8 level))
{
	...
	if (IS_HARDWARE_TYPE_8188E(adapter)){
		if(gpio_num > 7 || gpio_num < 4)
		{
			DBG_871X_LEVEL(_drv_info_, "%s The gpio number does not included 4~7.\n",__FUNCTION__);
			return -1;
		}
	}
	...
	direction = (rtw_read8(adapter,REG_GPIO_PIN_CTRL + 2) & BIT(gpio_num)) >> gpio_num;
	if(direction){
		DBG_871X_LEVEL(_drv_info_, "%s Can't register output gpio as interrupt.\n",__FUNCTION__);
		return -1;
	}
