...
void rtw_cfg80211_ibss_indicate_connect(_adapter *padapter)
{
	...
	if (!rtw_cfg80211_check_bss(padapter)) {
		...
		if (!rtw_cfg80211_check_bss(padapter))
			DBG_871X_LEVEL(_drv_info_, FUNC_ADPT_FMT" BSS not found !!\n", FUNC_ADPT_ARG(padapter));
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0))
int cfg80211_rtw_set_mac_acl(struct wiphy *wiphy, struct net_device *ndev,
                   const struct cfg80211_acl_data *params)
{
	...
	for (i = 0; i < params->n_acl_entries; i ++) {
		rtw_acl_add_sta(padapter, (u8 *)params->mac_addrs[i].addr);
	}
	...
	rtw_set_macaddr_acl(padapter, mode);
